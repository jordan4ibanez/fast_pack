...
ubyte* stbi__load_and_postprocess_8bit(stbi__context* s, int* x, int* y, int* comp, int req_comp) {
    ...
    if (ri.bits_per_channel != 8) {
        result = stbi__convert_16_to_8(cast(stbi__uint16*) result, *x, *y, req_comp == 0 ? *comp
                : req_comp);
...
stbi__uint16* stbi__load_and_postprocess_16bit(stbi__context* s, int* x, int* y, int* comp, int req_comp) {
    ...
    if (ri.bits_per_channel != 16) {
        result = stbi__convert_8_to_16(cast(stbi_uc*) result, *x, *y, req_comp == 0 ? *comp
                : req_comp);
...
static immutable stbi_uc[9] stbi__depth_scale_table = [
...
int stbi__create_png_image_raw(stbi__png* a, stbi_uc* raw, stbi__uint32 raw_len, int out_n, stbi__uint32 x, stbi__uint32 y, int depth, int color) {
    ...
    if (!stbi__mad3sizes_valid(img_n, x, depth, 7))
        return 0; //stbi__err("too large", "Corrupt PNG");
    ...
    if (raw_len < img_len)
        return 0; //stbi__err("not enough pixels","Corrupt PNG");
    ...
    for (j = 0; j < y; ++j) {
        ...
        if (depth < 8) {
            if (img_width_bytes > x)
                return 0; //stbi__err("invalid width","Corrupt PNG");
...
int stbi__parse_png_file(stbi__png* z, int scan, int req_comp) {
    ...
    for (;;) {
        ...
        case STBI__PNG_TYPE('I', 'H', 'D', 'R'): {
                int comp, filter;
                if (!first)
                    return 0; //stbi__err("multiple IHDR","Corrupt PNG");
                first = 0;
                if (c.length != 13)
                    return 0; //stbi__err("bad IHDR len","Corrupt PNG");
                s.img_x = stbi__get32be(s);
                s.img_y = stbi__get32be(s);
                if (s.img_y > STBI_MAX_DIMENSIONS)
                    return 0; //stbi__err("too large","Very large image (corrupt?)");
                if (s.img_x > STBI_MAX_DIMENSIONS)
                    return 0; //stbi__err("too large","Very large image (corrupt?)");
                z.depth = stbi__get8(s);
                if (z.depth != 1 && z.depth != 2 && z.depth != 4 && z.depth != 8 && z.depth != 16)
                    return 0; //stbi__err("1/2/4/8/16-bit only","PNG not supported: 1/2/4/8/16-bit only");
                color = stbi__get8(s);
                if (color > 6)
                    return 0; //stbi__err("bad ctype","Corrupt PNG");
                ...
                comp = stbi__get8(s);
                if (comp)
                    return 0; //stbi__err("bad comp method","Corrupt PNG");
                filter = stbi__get8(s);
                if (filter)
                    return 0; //stbi__err("bad filter method","Corrupt PNG");
                interlace = stbi__get8(s);
                if (interlace > 1)
                    return 0; //stbi__err("bad interlace method","Corrupt PNG");
                if (!s.img_x || !s.img_y)
                    return 0; //stbi__err("0-pixel image","Corrupt PNG");
                if (!pal_img_n) {
                    s.img_n = (color & 2 ? 3 : 1) + (color & 4 ? 1 : 0);
                    if ((1 << 30) / s.img_x / s.img_n < s.img_y)
                        return 0; //stbi__err("too large", "Image too large to decode");
        ...
        case STBI__PNG_TYPE('P', 'L', 'T', 'E'): {
                ...
                if (c.length > 256 * 3)
                    return 0; //stbi__err("invalid PLTE","Corrupt PNG");
                pal_len = c.length / 3;
        ...
        case STBI__PNG_TYPE('t', 'R', 'N', 'S'): {
                if (first)
                    return 0; //stbi__err("first not IHDR", "Corrupt PNG");
                if (z.idata)
                    return 0; //stbi__err("tRNS after IDAT","Corrupt PNG");
                if (pal_img_n) {
                    if (scan == STBI__SCAN_header) {
                        s.img_n = 4;
                        return 1;
                    ...
                    if (pal_len == 0)
                        return 0; //stbi__err("tRNS before PLTE","Corrupt PNG");
                    if (c.length > pal_len)
                        return 0; //stbi__err("bad tRNS len","Corrupt PNG");
                    pal_img_n = 4;
                    for (i = 0; i < c.length; ++i)
                        palette[i * 4 + 3] = stbi__get8(s);
                } else {
                    if (!(s.img_n & 1))
                        return 0; //stbi__err("tRNS with alpha","Corrupt PNG");
                    if (c.length != cast(stbi__uint32) s.img_n * 2)
                        return 0; //stbi__err("bad tRNS len","Corrupt PNG");
                    has_trans = 1;
                    if (z.depth == 16) {
                        for (k = 0; k < s.img_n; ++k)
                            tc16[k] = cast(stbi__uint16) stbi__get16be(s); // copy the values as-is
                    } else {
                        for (k = 0; k < s.img_n; ++k) {
                            tc[k] = cast(ubyte)(cast(stbi_uc)(stbi__get16be(
                                    s) & 255) * stbi__depth_scale_table[z.depth]); // non 8-bit images will be larger
                        }
                    }
                }
                break;
...
void* stbi__do_png(stbi__png* p, int* x, int* y, int* n, int req_comp, stbi__result_info* ri) {
    ...
    if (req_comp < 0 || req_comp > 4)
        return null; //stbi__errpuc("bad req_comp", "Internal error");
    if (stbi__parse_png_file(p, STBI__SCAN_load, req_comp)) {
        ...
        if (req_comp && req_comp != p.s.img_out_n) {
            if (ri.bits_per_channel == 8)
                result = stbi__convert_format(cast(ubyte*) result, p.s.img_out_n, req_comp, p.s.img_x, p
                        .s.img_y);
            else
                result = stbi__convert_format16(cast(stbi__uint16*) result, p.s.img_out_n, req_comp, p.s.img_x, p
                        .s.img_y);
...
version (decodeBMP) {
    ...
    void* stbi__bmp_parse_header(stbi__context* s, stbi__bmp_data* info) {
        ...
        if (hsz != 12) {
            ...
            if (compress == 1 || compress == 2)
                return null;
            if (compress >= 4)
                return null; // this includes PNG/JPEG modes
            if (compress == 3 && info.bpp != 16 && info.bpp != 32)
                return null; // bitfields requires 16 or 32 bits/pixel
            stbi__get32le(s); // discard sizeof

            int biXPelsPerMeter = stbi__get32le(s); // discard hres
            int biYPelsPerMeter = stbi__get32le(s); // discard vres
            if (biXPelsPerMeter > 1)
                s.ppmX = biXPelsPerMeter; // do not consider 0, 1, or negative values
            if (biYPelsPerMeter > 1)
                s.ppmY = biYPelsPerMeter; // do not consider 0, 1, or negative values
    ...
    void* stbi__bmp_load(stbi__context* s, int* x, int* y, int* comp, int req_comp, stbi__result_info* ri) {
        ...
        if (psize == 0) {
            ...
            int bytes_read_so_far = s.callback_already_read + cast(int)(
                s.img_buffer - s.img_buffer_original);
        ...
        if (info.bpp < 16) {
            ...
            for (j = 0; j < cast(int) s.img_y; ++j) {
                if (easy) {
                    ...
                    int bpp = info.bpp;
                    for (i = 0; i < cast(int) s.img_x; ++i) {
                        stbi__uint32 v = (bpp == 16 ? cast(stbi__uint32) stbi__get16le(s) : stbi__get32le(
                                s));
                        uint a;
        ...
        if (req_comp && req_comp != target) {
            ...
            if (out_ == null)
                return out_; // stbi__convert_format frees input on failure
        }
